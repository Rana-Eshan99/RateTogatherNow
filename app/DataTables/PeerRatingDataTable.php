...
class PeerRatingDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return datatables()
            ...
            ->editColumn('Overall', function ($row) {
                return $this->getOverAllPeerRating($row->id) ?? '--';
            })
            ->addColumn('action', function ($row) {
                return '<button class="btn float-left" style="padding: 0% !important" onclick="viewPeerRating(' . $row->id . ')">
     ...
     *
     * @param \App\Models\PeerRating $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(PeerRating $model)
    {
        return $model->where('status', PeerRatingStatusEnum::NEED_APPROVAL)->orderBy('createdAt', 'desc')->newQuery();
    }
    ...
    protected function getColumns()
    {
        return [
            ...
            Column::make('userId')->title('Given By')->orderable(false)->style('max-width: 120px;'),
            ...
            Column::make('experience')->title('Review')->style('min-width: 240px;')->orderable(false)->searchable(false),
    ...
    {
        return 'PeerRating_' . date('YmdHis');
    }
}
