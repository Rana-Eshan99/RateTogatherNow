...
class FeedbackDataTable extends DataTable
{
        ...
        return datatables()
            ...
            ->editColumn('userId', function ($row) {
                $formattedName = $row?->user ?
                    ...
                    'Anonymous';
                return mb_strimwidth($formattedName, 0, 30, "...");
            })
            ->editColumn('feeling', function ($row) {
                return ucwords(strtolower($row->feeling ?? '--'));;
            })
            ->editColumn('feedback', function ($row) {
                $feedback = $row->feedback;
                $truncatedFeedback = strlen($feedback) > 50 ? substr($feedback, 0, 50) . '...' : $feedback;
                return $truncatedFeedback;
            })
            ->addColumn('action', function ($row) {
                return '<button class="btn float-left" style="padding: 0% !important" onclick="viewFeedback(' . $row->id . ')">
            ...
            })->rawColumns(['action', 'userId', 'feeling', 'feedback'])
            ->filterColumn('feeling', function ($query, $keyword) {
                $query->where('feeling', 'like', "%{$keyword}%");
            })
     ...
     */
    public function query(ApplicationFeedback $model)
    {
        return $model->orderBy('createdAt', 'desc')->newQuery();
    }
    ...
    protected function getColumns()
    {
        return [
            ...
            Column::make('userId')->title('Given By')->style('max-width: 130px;')->orderable(false),
            Column::make('feeling')->title('Feeling')->orderable(false),
            Column::make('feedback')->orderable(false),
