        ...
        <div class="relative flex items-top justify-center min-h-screen bg-gray-100 dark:bg-gray-900 sm:items-center sm:pt-0">
            @if (Route::has('login'))
                <div class="hidden fixed top-0 right-0 px-6 py-4 sm:block">
                    ...
                    @else
                        <a href="{{ route('frontEnd.showSignIn') }}" class="text-sm text-gray-700 underline">FrontEnd-Login</a>

                        @if (Route::has('register'))
                            <a href="{{ route('frontEnd.showSignUp') }}" class="ml-4 text-sm text-gray-700 underline">FrontEnd-Sign Up</a>
                        @endif
                    @endauth
                </div>
            @endif
